...
sub vcl_recv {
  ...
  if (req.url ~ "^/oauth" || req.url ~ "^/resources") {
        set req.backend_hint = oauth;
        set req.http.host = "";
        set req.url = regsub(req.url, "^/oauth", "/");
        set req.url = regsub(req.url, "^/resources", "/");
        return(pipe);
  }
